...
\paragraph{Policy Iteration} By combining the two processes described before, we can derive an algorithm to sweep through all our states and upgrade our policy until the changes between each sweep is too small : it is controlled by a parameter $\epsilon$ and a parameter $\gamma$ (which we talked about previously).
...
The reason why this algorithm works is called the the \emph{policy improvement theorem}:
...
\begin{proof}
The idea of the proof is to expand the $q_{\pi}$ side until we get $v_{\pi'}(s)$ using : 
\begin{equation}
\begin{split}
q_{\pi}(s,a)
...
\item[16.03.2018] finished reading chapter 4 of Sutton's book and typed the end of the resume
