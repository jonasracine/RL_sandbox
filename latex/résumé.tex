...
\begin{proof} 
We define the Bellman operator : $\mathcal{T}_{\pi}:\mathbb{R}^{\mid \mathcal{S}\mid } \mapsto \mathbb{R}^{\mid \mathcal{S}\mid }$ in the following way :
\begin{equation*}
(\mathcal{T}_{\pi}V)(s)=\max_{a \in \mathcal{A}}\sum_{s'}\mathbb{P}(s'\mid a,s)\left[ R(s') + \gamma V(s') \right]
\end{equation*}
...
\begin{equation*}
\begin{split}
\mid \mathcal{T}_{\pi}V(s) - \mathcal{T}_{\pi}V'(s) \mid & \leq \mid \max_{a \in \mathcal{A}}\sum_{s'}\mathbb{P}(s'\mid a,s)\left[ R(s') + \gamma V(s') \right] - \max_{a \in \mathcal{A}}\sum_{s'}\mathbb{P}(s'\mid a,s)\left[ R(s') + \gamma V'(s') \right]\mid
...
Now using the property that a contraction has a unique fixed point and that all sequences $V,\mathcal{T}_{\pi}V,\mathcal{T}_{\pi}^2V,\ldots $ converges towards this fixed point we get the convergence.
